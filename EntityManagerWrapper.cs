        ...
        public void SetName(Entity entity, string name)
        {
            ...
            switch (Type)
            {
                case EntityManagerType.MOCK:
                    return;
                case EntityManagerType.ENTITY_MANAGER:
                    EntityManager.AddComponentData(entity, new SetName {value = new FixedString512Bytes(name)});
                    return;
                case EntityManagerType.ENTITY_COMMAND_BUFFER:
                case EntityManagerType.ENTITY_MANAGER_AND_COMMAND_BUFFER:
                    EntityCommandBuffer.AddComponent(entity, new SetName {value = new FixedString512Bytes(name)});
                    return;
                case EntityManagerType.ENTITY_COMMAND_BUFFER_CONCURRENT:
                    EntityCommandBufferConcurrent.AddComponent(EntityCommandBufferJobIndex, entity, new SetName {value = new FixedString64Bytes(name)});
                    return;
            }
